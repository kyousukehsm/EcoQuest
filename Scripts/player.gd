...
const LEVEL_MUSIC = {
	...
	"Level3": preload("res://Assets/Music/3-08 Zora_s Domain (Hyrule Symphony).mp3"),
}
const JUMP_SOUND = preload("res://Assets/Sounds/jump.wav")
const COLLECT_SOUND = preload("res://Assets/Sounds/collect.wav")
const HEAL_SOUND = preload("res://Assets/Sounds/get heal.wav")
...
func handle_scene_change(scene: Node) -> void:
	...
	if level_name.begins_with("Level"):
		audio_player.stop()  # Stop the current music before changing
		BackgroundMusic.pause_music()  # If using a separate manager or utility
...
func apply_gravity(delta: float) -> void:
	if is_in_water:
		...
		if velocity.y > 0:
			...
			velocity.y += current_gravity * delta * 0.9
...
func handle_swimming(delta: float) -> void:
	...
	# Clamp vertical velocity to prevent rapid sinking or rising
...
func heal(amount: int) -> void:
	...
	if health_bar:
		health_bar.value = health
		play_sfx(HEAL_SOUND)  # Play collect sound
	print("Health added: +%d. Current health: %d/%d" % [amount, health, max_health])
...
func _on_recyclable_collected() -> void:
	...
	if collected_count == total_collectibles:
		# Trigger level completion or allow transition to next level
		print("All collectibles gathered! You can proceed to the next level.")
		allow_level_transition()

func allow_level_transition() -> void:
	if collected_count == total_collectibles:
		print("Level complete! You can now move to the next level.")
		# You can now load the next level or trigger any level transition logic
		# Example: get_tree().change_scene("res://NextLevel.tscn")
	else:
		print("You need to collect all items to proceed.")
		# Optionally show a message or prevent transition (e.g., player cannot move to next level)
